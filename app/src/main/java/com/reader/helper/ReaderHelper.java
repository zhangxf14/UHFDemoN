...
public class ReaderHelper {
    ...
    public final static String BROADCAST_REFRESH_TEST_TEMP_PROGRESS = "com.reader.helper.refresh.progress";

    public final static String BROADCAST_REFRESH_TAG_MASK = "com.reader.helper.refresh.tag.mask";
    ...
    private void refreshTestTempProgress(int progress) {
        Intent intent = new Intent(BROADCAST_REFRESH_TEST_TEMP_PROGRESS);
        intent.putExtra("progress",progress);
        mLocalBroadcastManager.sendBroadcast(intent);
    }


    private void refreshTagMask(byte cmd,byte status) {
        Intent intent = new Intent(BROADCAST_REFRESH_TAG_MASK);
    ...
    private void analyData(MessageTran msgTran) {
        ...
        switch (msgTran.getCmd()) {
            ...
            case CMD.QUERY_READER_STATUS:
                processQueryReaderStatus(msgTran);
                break;
            case CMD.SET_READER_STATUS:
                processSetReaderStatus(msgTran);
                break;
            case CMD.FUDAN_COMMAND:
                processFuDanCommand(msgTran);
                break;
            default:
                break;
    ...
    private void processFuDanCommand(MessageTran msgTran) {

        byte btCmd = msgTran.getCmd();
        byte[] btAryData = msgTran.getAryData();
        String strCmd = CMD.format(btCmd);
        String strErrorCode = "";
        String[] operations = mContext.getResources().getStringArray(R.array.fundan_command_operation);

        switch (m_curOperateTagBuffer.fuDanOpCode) {
            case 0x01:
                strCmd = operations[0];
                break;
            case 0x02:
                strCmd = mContext.getResources().getString(R.string.start_rtc_temp);
                break;
            case 0x03:
                strCmd = operations[1];
                break;
            ...
            case 0x12:
                ...
                break;
            case 0x13:
                strCmd = mContext.getResources().getString(R.string.read_mem);
                break;
            case 0x14:
                strCmd = mContext.getResources().getString(R.string.write_mem);
                break;
            case 0x20:
                strCmd = operations[6];
                break;
        ...
        if (btAryData.length == 1) {
            ...
            if (m_curOperateTagBuffer.type == 0xF0 || m_curOperateTagBuffer.type == 0xF1 || m_curOperateTagBuffer.type == 0xF4
                || m_curOperateTagBuffer.type == 0xF5 || m_curOperateTagBuffer.type == 0xF6) {
                //RefreshOpFuDanTag(0x81);
                refreshOperateTag((byte)0x81,m_curOperateTagBuffer);
            }
            return;
        } else {
            int nLen = btAryData.length;
            byte code = btAryData[nLen - 3];
            int nDataLen = btAryData[2] & 0xFF;

            int nEpcLen = nDataLen - 4;

            String strPC = StringTool.byteArrayToString(btAryData, 3, 2);
            String strEPC = nEpcLen == 0 ? "" :StringTool.byteArrayToString(btAryData, 5, nEpcLen);
            String strCRC = StringTool.byteArrayToString(btAryData, 5 + nEpcLen, 2);

            if (m_curOperateTagBuffer.type == 0xF3 && code == 0x10) {
                m_curOperateTagBuffer.type = 0xF2;
                mReader.sendBuffer(m_curOperateTagBuffer.rtcTestTemp);
                return;
            }

            Log.d("STT:",String.format("Code:%02x fuDanCode:%02x type:%02x",code,m_curOperateTagBuffer.fuDanOpCode,m_curOperateTagBuffer.type));

            byte[] btData = null;
            String strData = "";
            if (code == 0x10 && (m_curOperateTagBuffer.fuDanOpCode == 0x01 ||
                    m_curOperateTagBuffer.fuDanOpCode == 0x10 ||
                    m_curOperateTagBuffer.fuDanOpCode == 0x11 || m_curOperateTagBuffer.fuDanOpCode == 0x13)) {

                btData = new byte[m_curOperateTagBuffer.readLen];
                System.arraycopy(btAryData, 3 + nDataLen - m_curOperateTagBuffer.readLen, btData, 0, btData.length);
                strData = StringTool.byteArrayToString(btData, 0, btData.length);
                strEPC = (nEpcLen - btData.length) == 0 ? "" : StringTool.byteArrayToString(btAryData, 5, nEpcLen - btData.length);
                strCRC = StringTool.byteArrayToString(btAryData, 5 + nEpcLen - btData.length, 2);
                if (m_curOperateTagBuffer.fuDanOpCode == 0x01) {
                    if (m_curOperateTagBuffer.type == 0x04) {
                        // byte[] test = new byte[2] {0x60,0x03 };
                        // Console.WriteLine("Tmp value:" + CCommondMethod.parseBitArrayToInt(new BitArray(test), 0, 10).ToString("#0.00"));
                        // Console.WriteLine("Tmp test:" + ((BitConverter.ToInt16(test, 0)) / 4.0).ToString("#0.00"));
                        byte[] tmp = new byte[2];
                        tmp[0] = btData[0];
                        tmp[1] = btData[1];
                        Log.d("KKKK",strData);
                        //Console.WriteLine("Tmp test:" + (CCommondMethod.parseBitArrayToInt(new BitArray(tmp),0,10) / 4.0).ToString("#0.00"));
                        strData = String.format("%.2f", NumberUtils.bitToInt(tmp, 10) / 4.0) + " ℃";
                        //strData = ((btData[0] * 256 + btData[1]) / 4.0).ToString("#0.00") + " ℃";
                        Log.d("KKKK",strData);
                    } else {
                        strData = String.format("%.2f", (btData[0] * 256 + btData[1]) / 8192.0 * 2.5) + " V";
                    }
                }

                if (m_curOperateTagBuffer.type == 0xF0) {
                    byte[] tmp = new byte[2];
                    tmp[0] = btData[2];
                    tmp[1] = btData[3];

                    int count = NumberUtils.byte2ToUnsignedShort(tmp);//BitConverter.ToUInt16(tmp, 0);

                    Log.d("KKKKK","count:" + count);

                    if (count != 0) {
                        //RefreshOpFuDanTag(0xF0);
                        //return;
                        m_curOperateTagBuffer.TempCount = count;

                    } else {

                        count = m_curOperateTagBuffer.TempCount;
                    }

                    m_curOperateTagBuffer.type = 0xF1;
                    m_curOperateTagBuffer.readLen = 32;
                    m_curOperateTagBuffer.readCount = count % 8 == 0 ? count / 8 : count / 8 + 1;
                    byte[] len = NumberUtils.intToByte4(32);
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 3] = len[2];
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 2] = len[3];

                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 1] =
                            MessageTran.checkSum(m_curOperateTagBuffer.readMemCmd, 0, m_curOperateTagBuffer.readMemCmd.length - 1);
                    mReader.sendBuffer(m_curOperateTagBuffer.readMemCmd);
                    m_curOperateTagBuffer.readCount--;
                    return;
                }

                if (m_curOperateTagBuffer.type == 0xF4)
                {
                    m_curOperateTagBuffer.timeStamp = NumberUtils.byte4ToInt(btData, 0);
                    m_curOperateTagBuffer.type = 0xF5;
                    m_curOperateTagBuffer.readLen = 4;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 5] = (byte)0xB0;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 4] = (byte)0x94;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 3] = (byte)0x00;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 2] = (byte)0x04;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 1] = MessageTran.checkSum(m_curOperateTagBuffer.readMemCmd, 0,
                            m_curOperateTagBuffer.readMemCmd.length - 1);
                    mReader.sendBuffer(m_curOperateTagBuffer.readMemCmd);
                    return;
                }

                if (m_curOperateTagBuffer.type == 0xF5)
                {
                    byte[] temp = new byte[2];
                    temp[0] = btData[1];
                    temp[1] = btData[0];

                    m_curOperateTagBuffer.testTempCount = NumberUtils.byte2ToUnsignedShort(temp);
                    m_curOperateTagBuffer.type = 0xF6;
                    m_curOperateTagBuffer.readLen = 4;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 5] = (byte)0xB0;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 4] = (byte)0x80;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 3] = (byte)0x00;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 2] = (byte)0x04;
                    m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 1] = MessageTran.checkSum(m_curOperateTagBuffer.readMemCmd, 0,
                            m_curOperateTagBuffer.readMemCmd.length - 1);
                ...
                if (m_curOperateTagBuffer.type == 0xF6)
                {
                    byte[] topTemp = new byte[2];
                    topTemp[0] = btData[3];
                    topTemp[1] = btData[2];
                    byte[] bottomTemp = new byte[2];
                    bottomTemp[0] = btData[1];
                    bottomTemp[1] = btData[0];
                    m_curOperateTagBuffer.type = 0xF3;

                    m_curOperateTagBuffer.topTemp = NumberUtils.bitToInt(topTemp,10) / 4.0;
                    m_curOperateTagBuffer.bottomTemp = NumberUtils.bitToInt(bottomTemp, 10) / 4.0;

                    File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "Temperature.txt");
                    try {
                        if (!file.exists()) {
                            file.createNewFile();
                        }
                        ...
                        try(OutputStream outputStream = new FileOutputStream(file,false)) {
                            for (Double temp : m_curOperateTagBuffer.points.values()) {
                    ...
                    //RefreshOpFuDanTag(0xF1);
                    refreshOperateTag((byte)0xF1,m_curOperateTagBuffer);
                    return;
                }



                if (m_curOperateTagBuffer.type == 0xF1) {
                    if (m_curOperateTagBuffer.readCount == 0) {
                        int count = m_curOperateTagBuffer.TempCount;
                        int baseNo = count % 8 == 0 ? count / 8 : count / 8 + 1;
                        int start = (baseNo - 1) * 8;
                        int len = count % 8 == 0 ? 8 : count % 8;
                        for (int i = 0; i < len; i++) {
                            byte[] point = new byte[]{btData[i * 4 + 1], btData[i * 4]};
                            m_curOperateTagBuffer.points.put(i + start, NumberUtils.bitToInt(point,10) / 4.0);
                        }

                        refreshTestTempProgress(m_curOperateTagBuffer.TempCount);
                        {
                            m_curOperateTagBuffer.type = 0xF4;
                            m_curOperateTagBuffer.readLen = 4;
                            m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 5] = 0x00;
                            m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 4] = 0x20;
                            m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 3] = 0x00;
                            m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 2] = 0x04;
                            m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 1] = MessageTran.checkSum(m_curOperateTagBuffer.readMemCmd, 0,
                                    m_curOperateTagBuffer.readMemCmd.length - 1);
                            mReader.sendBuffer(m_curOperateTagBuffer.readMemCmd);
                        }
                        ...
                        int count = m_curOperateTagBuffer.TempCount;
                        int baseNo = count % 8 == 0 ? count / 8 : count / 8 + 1;
                        int start = (baseNo - m_curOperateTagBuffer.readCount - 1) * 8;
                        int len = count % 8 == 0 ? 8 : count % 8;
                        for (int i = 0; i < 8; i++) {
                            byte[] point = new byte[]{btData[i * 4 + 1], btData[i * 4]};
                            m_curOperateTagBuffer.points.put(i + start, NumberUtils.bitToInt(point,10) / 4.0);
                        ...
                        byte[] readLen = NumberUtils.unsignedShortToByte2(32);
                        /*byte[] readLen = null;
                        if (m_curOperateTagBuffer.readCount == 1 && (m_curOperateTagBuffer.TempCount % 48 != 0)) {
                            readLen = NumberUtils.unsignedShortToByte2((m_curOperateTagBuffer.TempCount % 48));
                            m_curOperateTagBuffer.readLen = m_curOperateTagBuffer.TempCount % 48;
                        } else {
                            readLen  = NumberUtils.unsignedShortToByte2(48);
                        }*/

                        m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 3] = readLen[0];
                        m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 2] = readLen[1];
                        byte[] addBase = new byte[]{m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 5],
                                m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 4]};
                        int newAddBase = NumberUtils.byte2ToUnsignedShort(addBase, 0) + 32;
                        m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 5] = NumberUtils.unsignedShortToByte2(newAddBase)[0];
                        m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 4] = NumberUtils.unsignedShortToByte2(newAddBase)[1];
                        m_curOperateTagBuffer.readMemCmd[m_curOperateTagBuffer.readMemCmd.length - 1] =
                                MessageTran.checkSum(m_curOperateTagBuffer.readMemCmd, 0, m_curOperateTagBuffer.readMemCmd.length - 1);
                        mReader.sendBuffer(m_curOperateTagBuffer.readMemCmd);
                        m_curOperateTagBuffer.readCount--;
                        m_curOperateTagBuffer.testTempProgress += 8;
                        refreshTestTempProgress(m_curOperateTagBuffer.testTempProgress);
                        return;
                    }
                }
            }

            byte byTemp = btAryData[nLen - 2];
            byte byAntId = (byte) ((byTemp & 0x03) + 1);
            String strAntId = byAntId + "";

            String strReadCount = btAryData[nLen - 1] + "";

            String dataStatus = "";
            switch (code) {
                case 0x10:
                    dataStatus = mContext.getResources().getString(R.string.command_request_success);
                    break;
                case 0x3C:
                    dataStatus = mContext.getResources().getString(R.string.command_unknow);
                    break;
                case 0x3D:
                    dataStatus = mContext.getResources().getString(R.string.command_request_faild);
                    break;
                default:
                    dataStatus = ERROR.format(code);
                    break;
            }
            ...
            tag.strEPC = strEPC;
            tag.strData = strData;
            tag.nDataLen = dataStatus;
            tag.btAntId = byAntId;
            tag.nReadCount = btAryData[nLen - 1] & 0xFF;
            m_curOperateTagBuffer.lsTagList.add(tag);

            //refreshOperateTag(btCmd, m_curOperateTagBuffer);

            int status = code == 0x10 ? ERROR.SUCCESS : code;

            if (m_curOperateTagBuffer.type == 0xF0 || m_curOperateTagBuffer.type == 0xF1) {
                //RefreshOpFuDanTag(0x81);
                //WriteLog(lrtxtLog, dataStatus, status);
                writeLog(strCmd + " " + dataStatus, status);
                refreshOperateTag((byte)0x81,m_curOperateTagBuffer);
            } else {
                //RefreshOpFuDanTag(0xFF);
                //WriteLog(lrtxtLog, dataStatus, status);
                Log.d("KKKKK",strData + "   " + m_curOperateTagBuffer.lsTagList.size());
                writeLog(strCmd + " " + dataStatus, status);
                refreshOperateTag((byte)0x81,m_curOperateTagBuffer);
            }
        }
    ...
    private void processSetReaderStatus(MessageTran msgTran) {
        byte btCmd = msgTran.getCmd();
        byte[] btAryData = msgTran.getAryData();
        String strCmd = CMD.format(btCmd);
        String strErrorCode = "";
    ...
    private void processQueryReaderStatus(MessageTran msgTran) {
        byte btCmd = msgTran.getCmd();
        byte[] btAryData = msgTran.getAryData();
        String strCmd = CMD.format(btCmd);
        String strErrorCode = "";
        ...
        if (btAryData.length == 0x01) {
            m_curReaderSetting.btReadId = msgTran.getReadId();
            m_curReaderSetting.btReaderStatus = btAryData[0];
            refreshReaderSetting(btCmd, m_curReaderSetting);
            writeLog(strCmd, ERROR.SUCCESS);
            return;
        } else {
    ...
    private void processSetWorkAntenna(MessageTran msgTran) {
        ...
        String strCmd = mContext.getResources().getString(
                R.string.set_ant_success)
                + (m_curReaderSetting.btWorkAntenna + 1);
    ...
    private void processISO180006BInventory(MessageTran msgTran) {
        ...
        if (btAryData.length == 0x01) {
            ...
            ISO180006BOperateTagMap tag = null;
            Integer findIndex = m_curOperateTagISO18000Buffer.dtIndexMap
                    .get(strUID);
            if (findIndex == null) {
                tag = new ISO180006BOperateTagMap();
    ...
    private void processTagMask(MessageTran msgTran) {
        ...
        if (btAryData.length == 1) {
            if (btAryData[0] == (byte) 0x10) {
                ...
                refreshTagMask(msgTran.getCmd(),ERROR.SUCCESS);
                return;
        ...
        refreshTagMask(msgTran.getCmd(),ERROR.FAIL);
