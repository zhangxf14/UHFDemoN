		...
		mDataSendButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (result != null && result.length > 0) {
					MessageTran msg = new MessageTran(StringTool.stringArrayToByteArray(result, result.length));
					if (msg.getCmd() == CMD.SET_READER_STATUS) {
						if (msg.getAryData() != null) {
							m_curReaderSetting.btReaderStatus = msg.getAryData()[0];
						}
					}
